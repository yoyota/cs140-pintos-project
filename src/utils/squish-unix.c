...
static bool handle_error(ssize_t retval, int *fd, bool fd_is_sock,
			 const char *call)
{
	if (retval == 0) {
		if (fd_is_sock)
			...
			if (p->in != -1 && FD_ISSET(p->in, &read_fds)) {
				...
				if (n > 0) {
					...
					if (p->size == BUFSIZ && p->ofs != 0) {
						memmove(p->buf, p->buf + p->ofs,
							p->size);
						p->ofs = 0;
					}
				} else if (!handle_error(n, &p->in,
							 p->in == sock, "read"))
					return;
			}
...
int main(int argc __attribute__((unused)), char *argv[])
{
	...
	if (pid < 0)
		...
		for (;;) {
			...
			sigemptyset(&empty_set);
			retval = pselect(sock + 1, &read_fds, NULL, NULL, NULL,
					 &empty_set);
			if (retval < 0) {
				if (errno == EINTR)
					break;
