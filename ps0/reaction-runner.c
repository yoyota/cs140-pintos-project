...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < total_atoms; i++) {
		...
		if ((random() % 100) < hydrogen_pct) {
			hydrogen_atoms++;
			ret = pthread_create(&tid, NULL, hydrogen_thread,
					     &reaction);
		} else {
			oxygen_atoms++;
			ret = pthread_create(&tid, NULL, oxygen_thread,
					     &reaction);
		}
		if (ret != 0) {
	...
	printf("Created %d H and %d O atoms (%.1f%% H), expecting %d H2O molecules\n",
	       hydrogen_atoms, oxygen_atoms,
	       (double)hydrogen_atoms / total_atoms * 100.0,
	       expected_molecules);
