	       ...
	       void *aux)
{
	for (; *format != '\0'; format++) {
		...
		case 'i': {
			...
			format_integer(value < 0 ? -value : value, true,
		...
		case 's': {
			...
			format_string(s, strnlen(s, c.precision), &c, output,
				      aux);
		} break;
		...
		case 'p': {
			...
			c.flags = POUND;
			format_integer((uintptr_t)p, false, false, &base_x, &c,
				       output, aux);
		} break;
		...
		default:
			__printf("<<no %%%c conversion>>", output, aux,
				 *format);
			break;
		}
	}
...
static const char *parse_conversion(const char *format,
				    struct printf_conversion *c, va_list *args)
{
...
void print_human_readable_size(uint64_t size)
{
	if (size == 1)
		...
		static const char *factors[] = { "bytes", "kB", "MB",
						 "GB",	  "TB", NULL };
		const char **fp;
