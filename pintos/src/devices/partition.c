				 ...
				 int *part_nr)
{
	...
	if (sector >= block_size(block)) {
		printf("%s: Partition table at sector %" PRDSNu
		       " past end of device.\n",
			    ...
			    int part_nr)
{
	if (start >= block_size(block))
		...
		printf("%s%d: Partition end (%" PRDSNu
		       ") past end of device (%" PRDSNu ")\n",
		       block_name(block), part_nr, start + size,
		       block_size(block));
	else {
		...
		block_register(name, type, extra_info, size,
			       &partition_operations, p);
	}
}
