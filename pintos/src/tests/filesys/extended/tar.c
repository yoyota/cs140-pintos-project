			 ...
			 int archive_fd, bool *write_error)
{
	...
	if (file_fd >= 0) {
		...
		if (inumber(file_fd) != inumber(archive_fd)) {
			if (!isdir(file_fd))
				success = archive_ordinary_file(file_name,
								file_fd,
								archive_fd,
								write_error);
			else
				success = archive_directory(file_name,
							    file_name_size,
							    file_fd, archive_fd,
							    write_error);
		} else {
			      ...
			      int file_fd, int archive_fd, bool *write_error)
{
	...
	if (!write_header(file_name, USTAR_DIRECTORY, 0, archive_fd,
			  write_error))
		return false;

	file_name[dir_len] = '/';
	while (readdir(file_fd, &file_name[dir_len + 1]))
		if (!archive_file(file_name, file_name_size, archive_fd,
				  write_error))
			success = false;
	file_name[dir_len] = '\0';
...
static bool write_header(const char *file_name, enum ustar_type type, int size,
			 int archive_fd, bool *write_error)
{
